...
export default function NewChatModal({ leads, existingThreadIds, onClose, onOpenExisting, onStartNew }) {
  ...
  return (
    <div className="absolute inset-0 bg-black/50 flex items-center justify-center z-10" onClick={onClose}>
      <div className="w-[440px] bg-white rounded-xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-2.5 max-h-[200px] overflow-auto">
          {filtered.map((lead) => {
            const hasThread = existingThreadIds.includes(lead.id)
            return (
              <div key={lead.id} className="flex items-center gap-2.5 p-2 rounded-lg">
                <div
                ...
                <button
                  ...
                  className={
                    hasThread
                      ? 'text-[10.5px] font-semibold text-brand-indigo'
                      : 'text-[10.5px] font-bold text-white bg-brand-green px-2.5 py-1 rounded-md'
                  }
                >
