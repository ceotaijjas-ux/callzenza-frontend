...
import { ArrowLeft, RefreshCw, Printer, Download, Filter } from "lucide-react";
...
export default function GenericReportPage({ params }: ReportPageProps) {
  const { slug } = use(params);

  // Convert slug to clean Title (e.g., "inbound-summary-hourly" -> "Inbound Summary Hourly Report")
  ...
  const exportCSV = () => {
    ...
    const csvContent = [
      ...
      ...dataToExport.map((d) => `"${d.agent || d.agent_name || ''}","${d.date || ''}","${d.time || d.duration || ''}","${d.calls ?? 0}","${d.answered || 0}","${d.abandons || 0}","${d.status || ''}","${d.group || ''}"`),
    ].join("\n");
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-500 text-xs md:text-sm">
              Comprehensive report metrics, filtering and activity analysis
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-3.5 items-end">
            ...
            <div>
              ...
              >
                ...
                {Array.from(new Set(reportData.map((r: any) => r.agent_name || r.agent).filter(Boolean))).map((agent: any) => (
                ...
                <tbody className="divide-y divide-slate-100 text-xs">
                  {reportData.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-3.5 text-slate-600">{row.date}</td>
