...
export default function SystemStatusesPage() {
  ...
  const handleResetDefaults = async () => {
    if (
      !confirm(
        "Are you sure you want to restore the default system call dispositions? This will reset custom additions."
      )
    ) {
      return;
    }
  ...
  // Category counts
  const positiveCount = dispositions.filter((d) => d.category === "POSITIVE").length;
  const callbackCount = dispositions.filter((d) => d.category === "CALLBACK").length;
  const negativeCount = dispositions.filter((d) => d.category === "NEGATIVE").length;
  const systemCount = dispositions.filter((d) => d.category === "SYSTEM").length;
      ...
      <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <Link
              href="/admin"
              className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-indigo-600 transition-colors mb-1"
            >
            ...
            <div className="flex items-center gap-3">
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">
                Call Dispositions &amp; System Statuses
              </h1>
              <span className="text-[11px] font-extrabold uppercase px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1.5 shadow-2xs">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                Live Agent Sync
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium">
              Configure call disposition outcomes and status codes. Any addition or change made here immediately appears in the Voice Agent Dashboard disposition dropdown.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5">
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-2xs">
            <span className="text-[10px] uppercase font-extrabold text-slate-400">Total Configured</span>
            <div className="text-2xl font-black text-slate-900 mt-1">{dispositions.length}</div>
            <span className="text-[11px] text-slate-500 font-medium">All status codes</span>
          </div>

          <div className="bg-white border border-emerald-100 rounded-xl p-4 shadow-2xs">
            <span className="text-[10px] uppercase font-extrabold text-emerald-600">Positive / Sales</span>
            <div className="text-2xl font-black text-emerald-700 mt-1">{positiveCount}</div>
            <span className="text-[11px] text-emerald-600/80 font-medium">Deals &amp; interest</span>
          </div>

          <div className="bg-white border border-indigo-100 rounded-xl p-4 shadow-2xs">
            <span className="text-[10px] uppercase font-extrabold text-indigo-600">Callbacks</span>
            <div className="text-2xl font-black text-indigo-700 mt-1">{callbackCount}</div>
            <span className="text-[11px] text-indigo-600/80 font-medium">Scheduled &amp; busy</span>
          </div>

          <div className="bg-white border border-rose-100 rounded-xl p-4 shadow-2xs">
            <span className="text-[10px] uppercase font-extrabold text-rose-600">Negative / DNC</span>
            <div className="text-2xl font-black text-rose-700 mt-1">{negativeCount}</div>
            <span className="text-[11px] text-rose-600/80 font-medium">Declined / opt-out</span>
          </div>

          <div className="bg-white border border-amber-100 rounded-xl p-4 shadow-2xs">
            <span className="text-[10px] uppercase font-extrabold text-amber-600">System &amp; Audio</span>
            <div className="text-2xl font-black text-amber-700 mt-1">{systemCount}</div>
            <span className="text-[11px] text-amber-600/80 font-medium">VM / Disconnected</span>
          </div>
        </div>

        {/* Live Voice Agent Dropdown Preview Banner */}
        <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 rounded-xl p-5 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4 border border-indigo-800/40">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-indigo-300 text-xs font-black uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5 text-amber-400" />
              <span>Real-Time Voice Agent Dashboard Preview</span>
            </div>
            <p className="text-xs text-slate-300 font-medium">
              Below is exactly what Voice Agents currently see when selecting outcomes on completed calls:
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 bg-slate-800/80 border border-slate-700 px-3.5 py-2 rounded-xl">
            ...
            <select
              className="bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-1.5 text-xs font-semibold focus:outline-none"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title="Reset to factory default dispositions"
              >
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400">
                      No dispositions found matching the search or category filter.
                    </td>
                  </tr>
                ) : (
                  filtered.map((item) => (
